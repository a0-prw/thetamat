...
(defun init-mod-position-sys2-nat-lesson-slots ()
  (ps:ps 
    (defun init-mod-position-sys2-nat-lesson-slots ()
          ...
          (with-slots (get-inline-term show-summary quit-early
            ...
            (setf
             ...
             tio-grid-keypress ;; Slot done
              ...
              tutor-input-next
              (lambda (ev)
                (alert (strcat "NYI totor-input-next: " (id-of this))))
              ...
              ;;;;;;;;;;;;;;;;;;;;;;;;;; end from scratchpad
               ...
               (lambda (ev)
                 (let* (;(opr (with-tutor-state (tio-operator current-tutor-input)))
                        (vn (with-tutor-fsm current-variable)))
                         ;; (cond ((string= opr "+") "plus1:q")
                            ;;       ((string= opr "-") "sub1:q")
                            ;;       (t (alert (strcat "NYI: operator: " opr))))))
                   (funcall remove-tutor)
               ...
               summon-tutor-for-problem
               (lambda (v q a)
                 (setf (with-tutor-fsm current-variable) v)
...
(define-lesson mod-position-sys2-nat
    :step ((:p "This is a short lesson which explains about adding
    more than two longer numbers in the same way as adding two of
    them, and allows you to practice longer subtractions. It also
    explains (and lets you practice) the situation where you may have
    to repeatedly regroup because a higher power of ten digit is
    zero."))
    
    :step ((:p "If you have more than 2 numbers to add, you can set
    out your working in exactly the same way as you did with 2
    numbers.  Write them one under the other, with units lining up,
    tens lining up, etc, exactly as before.") (:p "Add the units
    digits; Write the units digits of your answer as the units digit
    of the final answer, and if the result was greater than 9, regroup
    by adding the excess digit to the tens; Exactly as before.  Repeat
    this process for the tens, hundreds, ... and so on.") (:p "The
    exercises have a problem of this sort.") (:p "On the next page,
    you can practice subtraction from a number with three digits."))

    :using-q&a-vars
    ((:q&a-var "sub1" subnat-lt1000)
    ...
    :step ((:p "When you subtract longer numbers, sometimes the
    situation can arise that you need to regroup, but the digit of the
    next higher power of ten is zero.") (:p "If this occurs, then just
    go to the next higher power again and regroup repeatedly along the
    number.") (:p "As before, turn on " (:q "hints") " and practice
    until you can do it without help."))
    
    :using-q&a-vars
    ((:q&a-var "sub2" subnat-z-100)
     (with-lesson-module-slots
         (funcall summon-tutor-for-problem 
                  "sub2:q"
                  (funcall use-var "sub2:q")
                  (funcall use-var "sub2:a"))))
    ...
    :step ((:h1 "Summary")
           ...
           (:ul (:li "Write down the first number.")
                ...
                (:li "If the digit of a higher power of ten is zero,
                repeat the process with an even higher power of ten
                and regroup along the number.")))
           (:p "So far, all the subtractions we have done have
           resulted in positive numbers, and we have merely mentioned
           the existence of negative numbers.  A later lesson will
           explain more about negative numbers, how they can arise,
           and some of the situations in which they can be useful
           in " (:q "the real world") ".")
